...
def video_result(    
    ...
    # Initialize Variable
    dataset_root, meta_data_root, cuda_device, torch_device = st.session_state.page_state["init"]
    # comapre = st.session_state.page_state["compare"]
    ...
    if "matches" not in st.session_state:
        st.session_state.matches = run_alignment(project, query, database)
    ...
    with title:
        st.title("Quetzal")
        if st.button("<- Back to Project Select View"):
            st.session_state.page_state["page"] = "file_system"
    ...
    with st.container(border=True):
        ...
        with imgc1:
            display_frame(
                label="Query Frame: " + query.getName(),
        ...
        with imgc2:
            if st.session_state.controller != "overlay":
                display_frame(
                    label="Aligned Database Frame: " + database.getName(),
