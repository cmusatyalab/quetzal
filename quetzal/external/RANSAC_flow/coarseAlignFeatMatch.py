...
from .utils import outil

class CoarseAlign:
    def __init__(self, nbScale, nbIter, tolerance, transform, minSize, segId = 1, segFg = True, imageNet = True, scaleR = 2, device = torch.device("cuda:0")):
    ...
    def setSource(self, Is_org) : 
        ...
        with torch.no_grad() : 
            ...
            self.IsTensor = self.toTensor(self.Is).unsqueeze(0).to(device=self.device)
            
            self.featsMultiScale = []
            self.WMultiScale = []
            self.HMultiScale = []
            for i in range(len(self.scaleList)) : 
                feat = F.normalize(self.net(self.preproc(IsList[i]).unsqueeze(0).to(device=self.device)))
    ...
    def setTarget(self, It_org) : 
        with torch.no_grad() : 
            ...
            self.ItTensor = self.toTensor(self.It).unsqueeze(0).to(device=self.device)
            self.featt = F.normalize(self.net(self.preproc(self.It).unsqueeze(0).to(device=self.device)))
    ...
    def getCoarse(self, Mt) : 
        ...
        with torch.no_grad() :  
            ...
            MtExtend = torch.from_numpy(MtExtend).to(device=self.device).unsqueeze(0).unsqueeze(0)
            ...
            ## RANSAC 
            ones = torch.cuda.FloatTensor(W1MutualMatch.size(0)).fill_(1).to(device=self.device)
