...
def non_example_routes():
    return [file for file in ft.get_directories(dataset_root) if not is_route_example(file)]
...
def delete_tab():
        ...
        with gr.Column(scale=4):
            ...
            with gr.Row():
                route_name_input = gr.Dropdown(choices=[file for file in ft.get_directories(dataset_root) if not is_route_example(file)], 
                            ...
                            value="str")
                video_name = gr.Dropdown(choices=[SELECT_ROUTE_MSG], 
                                label="Videos",
                                allow_custom_value=False, 
                                interactive=True,
                                value="str")
    ...
    route_name_input.change(get_video_files_for_route, inputs=route_name_input, outputs=[video_name, route_name_input])
...
def analyze_video(route_name, video_name, video_type: Literal["query", "database"], progress=gr.Progress(track_tqdm=True)):
    if route_name == "str":
...
def analyze_tab():
    with gr.Row():
        ...
        with gr.Column(scale=4):
            with gr.Row():
                route_name_input = gr.Dropdown(choices=non_example_routes(), 
                            label="Route", 
                            interactive=True,
                            allow_custom_value=False,
                            value="str")
                video_name = gr.Dropdown(choices=[SELECT_ROUTE_MSG], 
                                ...
                                value="str")
            with gr.Row():
                video_type_input = gr.Dropdown(choices=["database", "query"], 
                            label="Video Type", 
                            value="str",
                            interactive=True,
                            allow_custom_value=False)
    ...
    route_name_input.change(get_video_files_for_route, inputs=route_name_input, outputs=[video_name, route_name_input])
    analyze_btn.click(analyze_video, inputs=[route_name_input, video_name, video_type_input], outputs=[progress])
...
def upload_videos(videos, route_name, progress=gr.Progress(track_tqdm=True)):
    ...
    if not ft.is_valid_directory_name(route_name):
        ui_update["progress"] = "**ERROR: Invalid route name!** The route name must be '_' separated alphabets + digits"
        return ui_update, "-"
    ...
    for video in videos:
        ...
        try: move_video(video)
        except: return clean_transaction("**ERROR: Failed to upload Video!**")
...
def upload_tab():
    with gr.Row():
        ...
        with gr.Column(scale=4):
            ...
            with gr.Row():
                video_file = gr.File(label="Upload Video", file_count="multiple", type="filepath")
            with gr.Row():
                register_btn = gr.Button("Upload and Analyze")
    
    ui_updates = gr.State()
            
    register_btn.click(upload_videos, inputs=[video_file, route_name_input], outputs=[ui_updates, progress])
    route_name_input.change(update_non_example_routes, inputs=[], outputs=route_name_input)
    progress.change(update_upload_ui, inputs=ui_updates, outputs=[video_file, route_name_input, progress])
...
def run_dtw(route_name, database_video_name, query_video_name, progress):
    ...
    matches = [(query_frame_list[i], db_frame_list[i]) for i in tqdm(range(len(query_frame_list)))]
    ...
    db_indexes = create_FAISS_indexes(db_vlad.numpy(), cuda=(torch_device != torch.device("cpu")))
...
def update_ui_result(matches):
    if matches:
        ...
        query = matches[1]
        db = matches[2]
        return gr.update(visible=True), gr.update(visible=True, maximum=len(idx), value=0), gr.update(visible=True), query[idx[0][0]], db[idx[0][1]]
    else:
...
def get_analyzed_video_for_route(route):
    ...
    if route and route != "str":
        return get_analyzed_list(route, "database"), get_analyzed_list(route, "query"), gr.update(choices=all_routes)
    
    return gr.update(choices=[SELECT_ROUTE_MSG]), gr.update(choices=[SELECT_ROUTE_MSG]), gr.update(choices=all_routes)
...
def result_tab():  
        ...
        with gr.Column(scale = 6):      
            with gr.Row():
                route_name_input = gr.Dropdown(choices=ft.get_directories(dataset_root), 
                            ...
                            allow_custom_value=False,
                            value="str")
                database_video_name = gr.Dropdown(choices=[SELECT_ROUTE_MSG], 
                            label="Database Video",
                            allow_custom_value=False, 
                            interactive=True,
                            value="str")
                query_video_name = gr.Dropdown(choices=[SELECT_ROUTE_MSG], 
                            label="Query Video", 
                            allow_custom_value=False,
                            interactive=True,
                            value="str")
    ...
    with gr.Row():
        ...
        slider = gr.Slider(0, 1, value=1, step=1, label="Choose Query Frame Index", scale=4, visible=False)
    ...
    route_name_input.change(get_analyzed_video_for_route, inputs=route_name_input, outputs=[database_video_name, query_video_name, route_name_input])
    run_btn.click(run_matching, 
                  inputs=[route_name_input, database_video_name, query_video_name], 
                  outputs=[matching_states, prev_btn, slider, next_btn, query_img_orig, db_img_aligned, query_index], 
                  show_progress="full"
                  )
...
def main():
    ...
    parser.add_argument("--dataset-root", default="../data", help="Root directory of datasets")
