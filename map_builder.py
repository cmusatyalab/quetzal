...
def _create_map(coordinates, colors=None, dot_sizes=None, legend_names=None, map_intensity=0.6):
    dot_sizes = dot_sizes if dot_sizes is not None else [2] * len(coordinates)
    ...
    scatters = []
    fig = plt.figure(figsize=(map_img.shape[1]/200, map_img.shape[0]/200), dpi=500)
    ...
    if legend_names != None:
        plt.legend(scatters, legend_names, scatterpoints=50, loc='lower left',
                   ncol=1, framealpha=0, prop={"weight": "bold", "size": 3})
    ...
    map_img = cv2.resize(map_img[:, :, :], plot_img.shape[:2][::-1], interpolation=cv2.INTER_LANCZOS4)
    plot_img = cv2.resize(plot_img[:, :, :3], plot_img.shape[:2][::-1], interpolation=cv2.INTER_LANCZOS4)
...
def build_map_from_dataset(dataset_folder, dot_sizes=None, grouped_gps_coords=None):
    """dataset_folder is the path that contains the 'images' folder."""
    if grouped_gps_coords is None:
